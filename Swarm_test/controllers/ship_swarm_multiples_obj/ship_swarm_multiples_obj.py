"""ship_swarm_multiples_obj controller."""
...
def receive_msg():
    ...
    signal_strenght = rx.getSignalStrength()
    priority = list_message[0]
    pos_coord = list_message[1:3]
    
    return priority,pos_coord,signal_strenght
...
def main ():
    ...
    while robot.step(timestep) != -1:
        ...
        if state == "INITIALISATION":
            ...
            while (delay < 50 and robot.step(timestep) != -1):
                delay += 1
            print(robot.getName(),"running")
            
            zone_value = round(sensor[0].getValue()/1000.0,1)
...
# Receive the data from the broadcast            
        if state == "RECEVING":         
            priority,pos_coord,signal_strenght = receive_msg()
            rx.nextPacket()
            data_package(state)
...
# Calculate and orient himself to the objectif coordinates      
        if state == "CONVERGING": 
            ...
            if rx.getQueueLength() > 0:
                ...
                priority,new_pos_coord,new_signal_strenght = receive_msg()
                rx.nextPacket()
                if new_signal_strenght > signal_strenght:
                    print("Neareast signal spot...")
...
# The robot has found the area   
        if state == "FOUND":
            print(robot.getName(),"found source, broadcast...")
            emission_position(0) 
            data_package(state)
            new_state = "WAITING"
